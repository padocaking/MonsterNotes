...
const changeNavHover = (prevId, currId) => {
    document
        .getElementById(`monsterNav${prevId}`)
        .classList.remove("nav-item-hover");
    document
        .getElementById(`monsterNav${currId}`)
        .classList.add("nav-item-hover");
};
...
const renderHabitat = (habitatArr) => {
    for (let i = 0; i < 6; i++) {
        document.getElementById(`habitatImg${i}`).src =
            "../images/map_icons/none.png";
